...
import org.cwk.android.library.util.BroadcastUtil;
...
public class AutoLogin {
     ...
     * 如果未保存则不会执行，
     * 执行结束后会发送广播{@link org.cwk.android.library.global.ApplicationStaticValue
    ...
    public static void checkAutoLogin(Context context) {
        ...
        // 判断是否有用户名密码
        if (Global.getApplicationConfig().getUserName() != null && Global.getApplicationConfig()
                .getPassword() != null) {
            ...
            login.beginExecute(Global.getApplicationConfig().getUserName(), Global
                    .getApplicationConfig().getPassword());
        } else {
